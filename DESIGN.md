...
Docker 化していない。閲覧は Obsidian、判断は Telegram、検索とクラスタ俯瞰だけ自前の Web UI。
...
| メモとテーマの閲覧 | Obsidian | `repo/` を vault として開くだけ。バックリンク・グラフビューが無料で付く |
| 横断検索・クラスタ俯瞰・status 変更 | 自前 Web UI (FastAPI + HTML 1枚) | DB を引かないとできないことだけ |
